...
- El dashboard acepta únicamente cuentas de la lista `DASHBOARD_ALLOWED_USER_IDS` que además estén activas y tengan rol de oficina. Hoy entran `admin1` (Admin General), `carloszerme1` (Carlos) y `goreti` (Goreti). La Contadora y los repartidores no entran.
